...
const Archive: React.FC = () => {
  ...
  const driveImgUrl = (id: string) => `https://drive.google.com/uc?export=view&id=${id}`;
  ...
  const handleItemClick = (item: NavItem) => {
    setPlaybackError(null);
    if (item.type === 'folder') setCurrentPath([...currentPath, item.id]);
    else if (item.type === 'file' && item.data) openPlayer(item.data);
  };
    ...
    if (currentPath.length === 1) {
      ...
      return Object.keys(archiveTree[year] || {})
        ...
        .map((p) => {
          ...
          return { type: 'folder', name: p, id: p, coverId: firstEp?.coverId || '' };
        });
    }
  ...
  // COVER for header on program pages: take first episode's coverId (or none)
            ...
            <>
              {!programCoverId ? (
                <div className="mt-6 -mx-6 sm:-mx-8 h-48 bg-slate-800/50 rounded-b-3xl flex items-center justify-center text-slate-400 text-sm italic">
                  Sem capa para {currentPath[1]} (verifique Drive / CSV)
                </div>
              ) : (
                <div
                  ...
                  style={{
                    // ✅ FIX: url(...) not ur[](...)
                    backgroundImage: `linear-gradient(to bottom, rgba(0,0,0,0.45), rgba(0,0,0,0.75)), url(${driveImgUrl(
                      programCoverId
                    )})`,
